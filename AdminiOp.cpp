...
int AdminOp::Admin_Login(string number,vector<Admini> & x,string password)  //number是用户输入的工号  x是登陆成功后返回用户的个人信息
{
...
int AdminOp::SearchStu_number(string number,vector<Student> & x)    //按学号搜索学生信息 前端用户输入的学号传入number对象中
{
    //x存储学生信息
    int flag=copyInfo(number,Ad_number,x);  //学生信息存在x中，供前端使用
    return flag;   //flag为1表示搜索成功，反之失败
}
int AdminOp::SearchStu_name(string name,vector<Student> & x)      //按名字搜索学生名字    前端用户输入的姓名传入name对象中
{
    int flag=copyInfo(name,Ad_name,x);  //学生信息存在x中，供前端使用
    return flag;   //flag为1表示搜索成功，反之失败
}
int AdminOp::SearchTea_number(string number,vector<Teacher> &x)
{
    int flag=copyInfo(number,Tea_number,x);
    return flag;
}
int AdminOp::SearchTea_name(string name,vector<Teacher> &x)
{
    int flag=copyInfo(name,Tea_name,x);
    return flag;
}
int AdminOp::SearchCourse(string name,map<string,vector<Course>> &x,vector<string> & y)     //x中记录了课程文件名和课程信息的一一对应 y记录了所有课程文件名
{
...
int AdminOp::multi_Search(string cou_name,string tea_name,map<string,vector<Course>> &x,vector<string> &y)  //在教务员搜索课程信息时，支持两个关键字————》课程名字 老师
{
